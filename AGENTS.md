...
구현 또는 문서 수정을 시작하기 전에 다음을 먼저 확정합니다.
...
모든 변경 파일과 변경 라인은 현재 목표와 직접 연결되어야 합니다.
기존 구조로 해결할 수 있으면 새 abstraction, 계층, wrapper 또는 공통화를
추가하지 않습니다. 관련 없는 코드, 문서, 주석, 포맷 또는 파일명을 함께
정리하지 않습니다.

이미 제공된 사실은 다시 질문하지 않습니다. 작은 불확실성은 전제로 명시하고
진행하되, protected 영역 변경, scope 확장, secret 필요, 기준 HEAD 불일치 또는
필수 검증 불가능성이 확인되면 작업을 중단하고 그 이유를 보고합니다.

### AdMob 문제 진단 순서

광고 미노출 문제는 다음 범주를 먼저 구분합니다.

- SDK 초기화 실패
- UMP 또는 개인정보 동의 gate
- 광고 요청 미실행
- 테스트 광고와 production 광고 구성 차이
- 광고 재고 없음 또는 `NO_FILL`
- 네트워크, 기기, 지역, 계정 또는 정책 제한
- App ID 또는 광고 단위 ID 설정 오류

진단 근거 없이 Rewarded request/load/show 흐름이나 Android native 설정을
사전 수정하지 않습니다.
