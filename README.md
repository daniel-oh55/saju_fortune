...
하루풀이는 React + Vite + Capacitor Android 기반으로 운영되는 사주·운세 라이프스타일 앱입니다. Google Play 공개 상태와 내부 테스트 릴리스를 구분하여 관리하며, 사용자 프로필과 설정은 주로 `localStorage`에 저장합니다. 별도의 사용자 계정 서버나 로그인 시스템은 현재 사용하지 않습니다.
...
- 결제, 로그인, 사용자 계정 서버, 외부 분석 전용 SDK는 명시적인 별도 작업 승인 전까지 추가하지 않습니다.
