...
- Android adaptive icon foreground/background PNG가 `public/generated-icons/android-adaptive` 아래에 생성되었습니다.
- 생성 대상은 `public/brand/android-adaptive-icon-targets.json`을 기준으로 합니다.
- 생성 스크립트는 `npm run generate:android-adaptive-icons`입니다.
- 검증 스크립트는 `npm run check:generated-android-adaptive-icons`입니다.
