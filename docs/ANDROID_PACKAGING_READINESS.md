...
- release build, signing, AAB 생성 전 debug APK 설치 및 앱 실행 QA 결과를 확인해야 합니다.
...
- Android 앱 제출 전 공개 개인정보 처리방침 URL 확인 문서를 추가했습니다.
- 문서 경로는 `docs/PRIVACY_POLICY_URL_VERIFICATION.md`입니다.
- Vercel 배포 후 `/privacy/` 경로를 확인해야 합니다.

## 공개 개인정보 처리방침 페이지 브랜드명 보정

- `public/privacy/index.html`의 브랜드명을 `하루풀이` 기준으로 확인했습니다.
- Google Play 제출 전 `/privacy/` 페이지에서 `하루풀이` 표기가 정상 표시되는지 확인해야 합니다.
...
- 다음 검증 단계는 Android WebView localStorage QA입니다.
- QA 기준은 `docs/ANDROID_WEBVIEW_LOCALSTORAGE_QA.md`를 참고합니다.
- 이 단계에서는 release build, signing, Android 리소스 적용은 아직 진행하지 않습니다.
