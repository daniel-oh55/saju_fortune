...
- 공개 페이지와 데이터 보안 양식 초안이 충돌하지 않아야 합니다.
...
- 스토어 스크린샷의 샘플 문구는 데이터 보안 양식 초안과 충돌하지 않아야 합니다.
- `저장한 풀이` 화면에는 실제 사용자 프로필 원본 정보가 노출되지 않아야 합니다.
- 투자, 건강, 결제 압박처럼 오해를 줄 수 있는 단정적 표현은 사용하지 않습니다.
...
| 저장한 풀이 | localStorage | 사용자가 저장한 풀이 결과 보관 | 없음 | 민감 입력값 제외 원칙 유지 |
