...
## 2026-06-09 23시 이후 자시 기준 선택 옵션

- PR #26에서는 23시 이후 자시 정책을 전역으로 확정하지 않고 사용자 선택 옵션으로 제공한다.
- 기본값은 `same_day`이며, 입력한 날짜 기준으로 기존 계산 흐름을 유지한다.
- `next_day`는 사용자가 명시적으로 선택한 경우에만 적용한다.
- production 엔진은 `next_day` 선택 및 23:00~23:59 입력 조건이 모두 충족될 때만 계산 기준을 다음 날 00:분으로 보정한다.
- 태양시 보정은 여전히 적용하지 않는다.
- localStorage key 이름은 변경하지 않고, 계산 기준 선택이 fortune 캐시에 영향을 주므로 schemaVersion을 5로 올린다.

## 2026-06-09 23시 이후 출생 사용자 안내

- PR #25에서는 23시 이후 정책을 확정하지 않고 사용자 안내 문구만 추가한다.
- 23시 이후 출생자는 기준 차이 가능성을 온보딩 또는 프로필 확인 화면에서 안내한다.
- production 계산 결과는 변경하지 않는다.

## 2026-06-09 자시 후보와 외부 기준 비교

- PR #24에서 `1990-02-03 23:30`의 후보 계산값과 sky.told.me / 포스텔러 기준값을 비교했다.
- sky.told.me는 다음 날 자시 후보와 일치하지만, 포스텔러는 현재 후보 중 완전 일치하지 않는다.
- 따라서 23시 이후 기준은 아직 확정하지 않는다.
- 세 번째 기준 확인 전까지 production 엔진 변경은 보류한다.
...
하루풀리 만세력 엔진의 시간 경계, 절기 경계, 출생지, 태양시 보정, 자시 처리 기준을 문서화한다.
