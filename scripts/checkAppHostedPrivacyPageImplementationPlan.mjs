...
const resultDocPath = 'docs/APP_HOSTED_PRIVACY_PAGE_IMPLEMENTATION_PLAN.md';
...
if (!fs.existsSync(path.join(projectRoot, resultDocPath))) {
  console.error('App-hosted privacy page implementation plan check failed');
...
const requiredDocSnippets = [
  ...
  'Planned app-hosted page approach',
  '/privacy-policy',
  'Privacy page route/page implementation',
  'Not started',
  'Content requirements for actual page PR',
  ...
  '실제 결제 SDK 없음',
  '외부 분석 SDK 없음',
  'Verification plan after actual page implementation',
...
mark(
  developmentLogSource.includes('## App-hosted Privacy Page Implementation Plan'),
...
mark(
  packageJsonSource.includes(
    '"check:app-hosted-privacy-page-implementation-plan": "node scripts/checkAppHostedPrivacyPageImplementationPlan.mjs"',
  ),
...
console.log('App-hosted privacy page implementation plan check passed');
