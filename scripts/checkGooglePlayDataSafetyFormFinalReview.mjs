...
const finalReviewDocPath = 'docs/GOOGLE_PLAY_DATA_SAFETY_FORM_FINAL_REVIEW.md';
...
const requiredDocSnippets = [
  ...
  'Google Play Data safety form draft | Confirmed',
  'Data safety form final review | Confirmed',
...
const finalReviewDocExists = fs.existsSync(finalReviewDocPath);
logResult('google_play_data_safety_form_final_review_doc_exists', finalReviewDocExists, finalReviewDocPath);
if (!finalReviewDocExists) process.exit(1);

const docsToScan = [
  {
    path: finalReviewDocPath,
    source: fs.readFileSync(finalReviewDocPath, 'utf8'),
  },
];

if (!checkIncludes('final_review_doc', docsToScan[0].source, requiredDocSnippets)) hasFailure = true;
...
const scriptRegistered = packageJson.includes(
  '"check:google-play-data-safety-form-final-review": "node scripts/checkGooglePlayDataSafetyFormFinalReview.mjs"'
);
...
if (hasFailure) {
  console.error('Google Play Data safety form final review check failed');
  process.exit(1);
}

console.log('Google Play Data safety form final review check passed');
