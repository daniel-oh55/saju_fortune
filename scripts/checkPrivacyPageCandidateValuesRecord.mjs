...
const resultDocPath = 'docs/PRIVACY_PAGE_CANDIDATE_VALUES_RECORD.md';
...
if (!fs.existsSync(path.join(projectRoot, resultDocPath))) {
  console.error('Privacy page candidate values record check failed');
...
const requiredDocSnippets = [
  ...
  'User-provided candidate values',
...
mark(
  developmentLogSource.includes('## Privacy Page Candidate Values Record'),
...
mark(
  packageJsonSource.includes(
    '"check:privacy-page-candidate-values-record": "node scripts/checkPrivacyPageCandidateValuesRecord.mjs"',
  ),
...
console.log('Privacy page candidate values record check passed');
