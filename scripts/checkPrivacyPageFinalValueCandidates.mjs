...
const resultDocPath = 'docs/PRIVACY_PAGE_FINAL_VALUE_CANDIDATES.md';
...
if (!fs.existsSync(path.join(projectRoot, resultDocPath))) {
  console.error('Privacy page final value candidates check failed');
...
const requiredDocSnippets = [
  ...
  'Candidate values to be provided by user',
  'Candidate recording rules',
  'Verification required after candidates are provided',
...
mark(
  developmentLogSource.includes('## Privacy Page Final Value Candidates'),
...
mark(
  packageJsonSource.includes(
    '"check:privacy-page-final-value-candidates": "node scripts/checkPrivacyPageFinalValueCandidates.mjs"',
  ),
...
console.log('Privacy page final value candidates check passed');
