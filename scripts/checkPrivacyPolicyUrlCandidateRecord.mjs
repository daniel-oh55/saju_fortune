...
const resultDocPath = 'docs/PRIVACY_POLICY_URL_CANDIDATE_RECORD.md';
...
if (!fs.existsSync(path.join(projectRoot, resultDocPath))) {
  console.error('Privacy policy URL candidate record check failed');
...
const requiredDocSnippets = [
  ...
  'Correct path check',
  'Content matches current app state',
...
mark(
  packageJsonSource.includes(
    '"check:privacy-policy-url-candidate-record": "node scripts/checkPrivacyPolicyUrlCandidateRecord.mjs"',
  ),
...
console.log('Privacy policy URL candidate record check passed');
