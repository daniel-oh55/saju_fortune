...
const resultDocPath = 'docs/PRIVACY_POLICY_URL_VERIFICATION_RECORD.md';
...
if (!fs.existsSync(path.join(projectRoot, resultDocPath))) {
  console.error('Privacy policy URL verification record check failed');
...
const requiredDocSnippets = [
  ...
  'Purpose: Record verification results for the privacy policy URL candidate before finalization',
  ...
  'Correct path check /privacy-policy/',
  'support.hym@gmail.com visible',
  '2026년 7월 12일 visible',
  '하루풀이 운영자 visible',
  'support.hym@gmail.com test email received',
  'Finalization blockers still remaining',
...
mark(!resultDoc.includes('<USER_CONFIRMED_YES_OR_NO>'), 'result_doc_no_unresolved_placeholder');
...
for (const item of verificationResultItems) {
  const rowPattern = new RegExp(`\\| ${item.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')} \\| (YES|NO) \\|`);
  mark(rowPattern.test(resultDoc), `result_doc_has_recorded_result_${item}`);
}
...
mark(
  packageJsonSource.includes(
    '"check:privacy-policy-url-verification-record": "node scripts/checkPrivacyPolicyUrlVerificationRecord.mjs"',
  ),
...
console.log('Privacy policy URL verification record check passed');
