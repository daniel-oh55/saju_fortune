...
const docPath = 'docs/RELEASE_WORKFLOW_DESIGN.md';
...
const relatedDocSnippet = 'Release workflow design: docs/RELEASE_WORKFLOW_DESIGN.md';
...
const releaseChecklistHasRelatedDoc = releaseChecklist.includes(relatedDocSnippet);
logResult('release_build_signing_checklist_includes_release_workflow_related_doc', releaseChecklistHasRelatedDoc);
if (!releaseChecklistHasRelatedDoc) hasFailure = true;
...
const workflowDiffOutput = execSync(`git diff --name-only -- ${workflowFiles.join(' ')}`, {
  encoding: 'utf8',
}).trim();
const workflowFilesUnchanged = workflowDiffOutput.length === 0;
logResult('github_actions_workflow_files_unchanged_in_working_diff', workflowFilesUnchanged);
if (!workflowFilesUnchanged) hasFailure = true;

const androidDiffOutput = execSync(`git diff --name-only -- ${androidNativeGradleSigningFiles.join(' ')}`, {
  encoding: 'utf8',
}).trim();
const androidNativeGradleSigningFilesUnchanged = androidDiffOutput.length === 0;
...
if (hasFailure) {
  console.error('Release workflow design check failed');
  process.exit(1);
}

console.log('Release workflow design check passed');
