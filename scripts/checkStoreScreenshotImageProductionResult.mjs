...
const resultDocPath = 'docs/STORE_SCREENSHOT_IMAGE_PRODUCTION_RESULT.md';
...
if (failedBeforeDocRead.length > 0) {
  console.error('Store screenshot image production result check failed');
...
const requiredDocSnippets = [
  ...
  'Privacy and content safety result',
  'Remaining Google Play items',
  'No src changes',
  'No CSS changes',
...
mark(
  developmentLogSource.includes('## Store Screenshot Image Production Result'),
...
mark(
  packageJsonSource.includes(
    '"check:store-screenshot-image-production-result": "node scripts/checkStoreScreenshotImageProductionResult.mjs"',
  ),
...
console.log('Store screenshot image production result check passed');
