...
const planDocPath = 'docs/STORE_SCREENSHOT_PRODUCTION_PLAN.md';
...
mark(fs.existsSync(path.join(projectRoot, planDocPath)), 'plan_doc_exists');
if (!fs.existsSync(path.join(projectRoot, planDocPath))) {
  console.error('Store screenshot production plan check failed');
  console.error('- plan_doc_exists');
  process.exit(1);
}
...
for (const snippet of requiredDocSnippets) {
  mark(planDoc.includes(snippet), `plan_doc_includes_${snippet}`);
}
...
mark(!forbiddenSnippets.some((snippet) => planDoc.includes(snippet)), 'plan_doc_no_forbidden_snippets');
...
mark(developmentLogSource.includes('## Store Screenshot Production Plan'), 'development_log_has_section');
mark(
  changelogSource.includes('Documented store screenshot production plan for Google Play launch preparation.'),
  'changelog_records_plan_doc',
);
...
console.log('Store screenshot production plan check passed');
