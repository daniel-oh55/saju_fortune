...
const docPath = 'docs/YEAR_MONTHLY_FORTUNE_IMPLEMENTATION_PLAN.md';
const relatedDocs = [
...
const requiredDocSnippets = [
  ...
  'src/domain/fortune/yearFortuneEngine.js | improve annual/monthly score and text composition in a small scoped PR',
  ...
  'Today fortune output unchanged | CURRENT_FORTUNE_SCHEMA_VERSION should not change for year/monthly-only PR',
...
if (!checkIncludes('year_monthly_implementation_plan_doc', doc, requiredDocSnippets)) {
  hasFailure = true;
}
...
const scriptRegistered = packageJson.includes(
  '"check:year-monthly-fortune-implementation-plan": "node scripts/checkYearMonthlyFortuneImplementationPlan.mjs"'
);
...
if (hasFailure) {
  console.error('Year/monthly fortune implementation plan check failed');
  process.exit(1);
}

console.log('Year/monthly fortune implementation plan check passed');
