...
const docPath = 'docs/YEAR_MONTHLY_SNAPSHOT_COMPARISON_CHECK_DESIGN.md';
const relatedDocs = [
...
const requiredDocSnippets = [
  ...
  'docs/generated/fortune-engine-sample-snapshot-after-today-improvement.json | practical baseline before year/monthly improvement',
  ...
  'todayFortuneSummary | unchanged from after-today baseline',
  'yearFortuneSummary | expected to change with review',
  'monthly entries count | 12 monthly entries preserved',
  'monthly entries count changed | Not allowed',
  'today fortune output changed | Not allowed',
  'zodiac fortune output changed | Not allowed',
...
if (!checkIncludes('year_monthly_snapshot_comparison_design_doc', doc, requiredDocSnippets)) {
  hasFailure = true;
}
...
const scriptRegistered = packageJson.includes(
  '"check:year-monthly-snapshot-comparison-check-design": "node scripts/checkYearMonthlySnapshotComparisonCheckDesign.mjs"'
);
...
if (hasFailure) {
  console.error('Year/monthly snapshot comparison check design failed');
  process.exit(1);
}

console.log('Year/monthly snapshot comparison check design passed');
