...
const docPath = 'docs/ZODIAC_FORTUNE_ENGINE_IMPROVEMENT_DESIGN.md';
const relatedDocs = [
...
const requiredDocSnippets = [
  ...
  'Improve animal-specific tone | Pending design',
  'Improve category-specific guidance | Pending design',
  'selectedAnimal | animal-specific tone and summary',
  'existing category IDs preserved | Required',
  'today fortune output unchanged | Required',
  'year/monthly fortune output unchanged | Required',
  'manseryeok output unchanged | Required',
  'saju analysis output unchanged | Required',
...
if (!checkIncludes('zodiac_design_doc', doc, requiredDocSnippets)) hasFailure = true;
...
const scriptRegistered = packageJson.includes(
  '"check:zodiac-fortune-engine-improvement-design": "node scripts/checkZodiacFortuneEngineImprovementDesign.mjs"'
);
...
if (hasFailure) {
  console.error('Zodiac fortune engine improvement design check failed');
  process.exit(1);
}

console.log('Zodiac fortune engine improvement design check passed');
