...
const outputPath = 'docs/generated/fortune-engine-sample-snapshot-after-today-improvement.json';
...
async function main() {
  ...
  try {
    ...
    const samples = baseline.samples.map((baselineSample) => {
    ...
    const snapshot = {
      snapshotVersion: 'fortune_engine_sample_snapshot_after_today_improvement_v1',
      dateKey,
      targetYear,
      source: 'artificial_sample_profiles_only',
      productionLogicChanged: true,
      productionLogicChangeScope: 'today_fortune_first_scope_only',
    ...
    console.log(`Today fortune after snapshot written to ${outputPath}`);
  } finally {
    await server.close();
  }
}
