...
const outputPath = 'docs/generated/fortune-engine-sample-snapshot-after-year-monthly-improvement.json';
...
async function main() {
  ...
  try {
    ...
    const snapshot = {
      snapshotVersion: 'fortune_engine_sample_snapshot_after_year_monthly_improvement_v1',
      source: 'artificial_sample_profiles_only',
      productionLogicChanged: true,
      productionLogicChangeScope: 'year_monthly_first_scope_only',
    ...
    console.log(`Year/monthly fortune after snapshot written to ${outputPath}`);
  } finally {
    await server.close();
  }
}
